...
locals {
  ...
  playbook_skip_tags = var.playbook_skip_tags != "" ? "--skip-tags=${var.playbook_skip_tags}" : ""

  command = <<EOC
    ansible-playbook -i ${abspath("${path.module}/inventory.yaml")} \
        ${var.playbook} -b --private-key ${abspath("${path.module}/ssh_key")} \
        ${local.playbook_tags} ${local.playbook_vars} ${local.playbook_skip_tags} ${var.ansible_verbose}
    EOC
